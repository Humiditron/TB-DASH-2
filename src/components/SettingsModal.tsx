...
import { X, Server, ShieldCheck, Check, Key, Globe, Lock, Cpu } from 'lucide-react';
...
export const SettingsModal: React.FC<SettingsModalProps> = ({
  ...
  const [isSimulated, setIsSimulated] = useState(config.isSimulated);
  ...
  const handleSaveAndConnect = async (e: React.FormEvent) => {
    ...
    if (!isSimulated && username && password) {
      const success = await onLogin(username, password);
      setIsConnecting(false);
      if (success) {
        setStatusFeedback('Connected successfully to ThingsBoard CE!');
        setTimeout(() => onClose(), 1200);
      } else {
        setStatusFeedback('Connection error: Please verify URL & credentials (or use Simulated Mode).');
      }
    } else {
      setIsConnecting(false);
      setStatusFeedback('Configuration saved.');
    ...
    <div id="modal-settings" className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-slate-950/80 backdrop-blur-xs">
      <div className="bg-slate-900 border border-slate-700 rounded-2xl w-full max-w-lg p-6 shadow-2xl space-y-5 animate-scale-in">
        ...
        <div className="flex items-center justify-between border-b border-slate-800 pb-3">
          <div className="flex items-center gap-2.5">
            ...
            <div>
              ...
              <p className="text-xs text-slate-400">Manage REST/WebSocket API endpoints and authentication</p>
            </div>
          </div>
        ...
        {statusFeedback && (
          <div
            className={`p-3 rounded-lg text-xs font-semibold flex items-center gap-2 ${
              statusFeedback.includes('successfully') || statusFeedback.includes('saved')
        ...
        <form onSubmit={handleSaveAndConnect} className="space-y-4">
          {/* Mode Switcher: Simulated vs Live */}
          <div className="bg-slate-950 p-3.5 rounded-xl border border-slate-800 flex items-center justify-between">
            <div>
              <span className="text-xs font-bold text-slate-200 block">Telemetry Simulation Mode</span>
              <span className="text-[11px] text-slate-400">
                Generates live sensor fluctuations & alerts in sandbox
              </span>
            </div>
            ...
            <button
              ...
              className={`relative inline-flex h-6 w-11 shrink-0 cursor-pointer rounded-full border-2 border-transparent transition-colors duration-200 ease-in-out focus:outline-none ${
                isSimulated ? 'bg-amber-600' : 'bg-slate-800'
              }`}
            >
              <span
          ...
          <div>
            ...
            <div className="relative">
              <input
                ...
                placeholder="https://humid1.yourdomain.com"
            ...
            <span className="text-[11px] text-slate-500 mt-1 block">
              Default proxy routes to ThingsBoard CE on port 8080 with Caddy CORS
            </span>
          </div>
